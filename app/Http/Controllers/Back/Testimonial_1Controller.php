...
use App\Models\Testimonial_1;
use Illuminate\Http\Request;

class Testimonial_1Controller extends Controller
{
    public function index(TestimonialsDataTable $dataTable)
    {
        if (\Auth::user()->can('manage-testimonial')) {
            $testimonials = Testimonial_1::orderBy('id', 'desc')->get();
    ...
    public function edit($id)
    {
        if (\Auth::user()->can('edit-testimonial')) {
            $testimonial = Testimonial_1::find($id);
    ...
    {
        if (\Auth::user()->can('edit-testimonial')) {
            $updateTestimonial = Testimonial_1::find($id);
            if ($request->hasfile('image')) {
    ...
    public function destroy($id)
    {
        if (\Auth::user()->can('delete-testimonial')) {
            $deleteTestimonial = Testimonial_1::find($id);
